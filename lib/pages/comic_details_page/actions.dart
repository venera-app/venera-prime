...
abstract mixin class _ComicPageActions {
  ...
  void download() async {
    ...
    if (comic.chapters == null) {
      ...
      if (localComic != null) {
        for (final rawIndex in visible) {
          if (localComic.downloadedChapters.contains(
            comic.chapters!.ids.elementAt(rawIndex),
          )) {
            downloaded.add(visible.indexOf(rawIndex));
          }
        }
      }
      await showSideBar(
        ...
        _SelectDownloadChapter(
          visible.map((i) => comic.chapters!.titles.elementAt(i)).toList(),
          (v) => selected = v.map((i) => visible[i]).toList(),
          downloaded,
        ),
      );
  ...
  void showMoreActions() {
    ...
    showMenuX(context, Offset(context.width - 16, context.padding.top), [
      MenuEntry(
        icon: ReadLaterManager().contains(comic.id, comic.comicType)
            ? Icons.bookmark_remove_outlined
            : Icons.watch_later_outlined,
        text:
            (ReadLaterManager().contains(comic.id, comic.comicType)
                    ? 'Remove from read later'
                    : 'Read later')
                .tl,
        onClick: () {
          final added = !ReadLaterManager().contains(comic.id, comic.comicType);
          ...
          if (added) {
            ReadLaterManager().add(snapshot);
          } else {
            ReadLaterManager().remove(comic.id, comic.comicType);
          }
          context.showMessage(
            message:
                (added ? 'Added to read later' : 'Removed from read later').tl,
          );
        },
      ),
      MenuEntry(
